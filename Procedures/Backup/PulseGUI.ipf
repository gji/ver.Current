...
//_____________________________________________________________________________
//
//	ComCheck(port) checks the error status of a COM port before using it
//_____________________________________________________________________________
//
...
function ComCheck()
	VDTGetStatus2	0, 1, 1
	if((V_VDT %& 1) != 0)
		print " software overrun error occurred. A software overrun occurs if the serial driver input buffer is filled up and more characters arrive from the sender."
		return -1
	elseif((V_VDT %& 8) != 0)
		print "The break signal was asserted."
		return -1
	elseif((V_VDT %& 16) != 0)
		print "A parity error occurred."
		return -1
	elseif((V_VDT %& 32) != 0)
		print "A hardware overrun error occurred. This means that the receiver has received a character before the preceding character was read by the serial driver."
		return -1
	elseif((V_VDT %& 64) != 0)
		print " A framing error occurred. The most likely cause for a framing error is that you have not configured the VDT2 baud rate or stop bits settings to match the baud rate and stop bits used by the device that you are talking to."
		return -1
	elseif((V_VDT %& 256) != 0)
		print "parallel device not selected"
		return -1
	elseif((V_VDT %& 512) != 0)
		print "general I/O error" 
		return -1
	elseif((V_VDT %& 1024) != 0)
		print "requested mode not supported"
		return -1
	elseif((V_VDT %& 2048) != 0)
		print "parallel device out of paper"
		return -1
	elseif((V_VDT %& 4096) != 0)
		print "parallel device timed out"
		return -1
	elseif((V_VDT %& 8192) != 0)
...
//_____________________________________________________________________________
//
//	RunExperiment(expt) creates an Experiment structure based on the current sequence type
//_____________________________________________________________________________
//
Function RunExperiment(expt)
	...
	for (i=0;i<expt.numExOps;i+=1)
		cOp = expt.ExOps[i]
		if(cOp.Scanned == 1)
			expt.ExOps[i].Values = ReplaceListItem(cOp.Values, num2str(cOp.ScanStart), cOp.ScanParameter)
			...
			if(stringmatch("Duration",ScanVarName))
				dataScanVar[(DimSize(dataScanVar,0)-1)]	= cOp.ScanStart*10^-6
			else
				dataScanVar[(DimSize(dataScanVar,0)-1)]	= cOp.ScanStart
			endif
		endif
	endfor
	...
	do
		for (i=0;i<expt.numExOps;i+=1)
			cOp = expt.ExOps[i]
			if(cOp.Scanned == 1)
				Variable nVal = str2num(StringFromList(cOp.ScanParameter, cOp.Values)) + cOp.ScanInc
				if(nVal > cOp.ScanStop)
					...
					Redimension/N=(Dimsize(dataScanVar,0)+1) dataScanVar
					if(stringmatch("Duration",ScanVarName))
						dataScanVar[(DimSize(dataScanVar,0)-1)]	= nVal*10^-6
					else
...
Function/WAVE RunExpValues(expt)
	...
	WAVE DDSNames = root:ExpParams:DDSNames
	WAVE TTLNames = root:ExpParams:TTLNames
	WAVE NameWave = root:ExpParams:NameWave
	WAVE OverrideWave = root:ExpParams:OverrideWave
	...
	Make/O/N=(expt.numExOps,2)/I/U fpgaSeq // unsigned 32-bit
	STRUCT ExOp cOp;
	for (i=0;i<expt.numExOps;i+=1)
		cOp = expt.ExOps[i]
		
		FindValue/TEXT=cOp.description TTLNames
		Variable ttlNo = V_value
		fpgaSeq[i][0] = (NameWave[ttlNo] & ~mask) | enable
		fpgaSeq[i][1] = str2num(StringFromList(0, cOp.Values))
		strswitch(cOp.device[0,2])
			case "DDS":
				...
				if(DDS_INFO[ddsNo][3] == 0) // if not overridden
					...
					//print ddsNo
					setDDS(ddsNo+1,frequency*1000000, 0, amplitude) // dds numbers are 1 indexed. freq in hertz
				endif
			break
		endswitch
	endfor
	...
	sendSequence(fpgaSeq)
	//wave out = runSequence(expt.exptsPerPoint, recmask = 0x01000000)
	return runSequence(expt.exptsPerPoint, recmask = 0x01000000)
	//VDTClosePort2 $seq_p	
